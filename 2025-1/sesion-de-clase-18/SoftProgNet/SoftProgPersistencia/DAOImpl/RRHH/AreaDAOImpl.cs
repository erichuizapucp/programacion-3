...
namespace PUCP.Edu.Pe.SoftProg.Persistencia.DAOImpl.RRHH {
    public class AreaDAOImpl : BaseDAOImpl<Area>, IAreaDAO {
        ...
        protected override DbCommand CommandoInsertar(DbConnection conn, DbTransaction transaccion, Area area) {
            DbCommand cmd = this.CommandoInsertar(conn, area);
            cmd.Transaction = transaccion;
            return cmd;
        }

        protected override DbCommand CommandoModificar(DbConnection conn, Area area) {
            DbCommand cmd = conn.CreateCommand();
