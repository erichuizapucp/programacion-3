...
public abstract class PersonaBaseDao<M> : DefaultBaseDao<M>, IPersonaDao<M> where M : Persona
{
    ...
    {
        modelo.Dni = reader.GetString(reader.GetOrdinal("dni"));
        modelo.Nombre = reader.GetString(reader.GetOrdinal("nombre"));
        modelo.ApellidoPaterno = reader.GetString(reader.GetOrdinal("apellidoPaterno"));
        modelo.Genero = Enum.Parse<Genero>(reader.GetString(reader.GetOrdinal("genero")));
        modelo.FechaNacimiento = reader.GetDateTime(reader.GetOrdinal("fechaNacimiento"));
    }
